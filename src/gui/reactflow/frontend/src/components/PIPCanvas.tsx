...
/**
 * 画像上でドラッグしてPIP領域を指定するキャンバス。
 * 左ドラッグで矩形を描画し、離すとmin_x/min_y/max_x/max_yを更新する。
 */
export default function PIPCanvas({
  imageData,
  minX,
  minY,
  maxX,
  ...
  const handleMouseUp = useCallback(
    ...
    [isDragging, dragStart, dragEnd, onRegionChange]
  );
  ...
  return (
    ...
    >
      ...
      {/* PIP領域のオーバーレイ（画像コンテンツ領域に配置） */}
      {showSelection && (
        <div
          className="pip-selection"
          style={{
            position: 'absolute',
            ...getSelectionStyle(),
