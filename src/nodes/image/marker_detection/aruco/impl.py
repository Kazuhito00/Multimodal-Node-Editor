...
class ArucoLogic(ComputeLogic):
    """
    ArUcoマーカー検出ノードロジック。
    cv2.aruco.ArucoDetectorを使用してArUcoマーカーを検出。
    """
    ...
    def compute(
        ...
        dict_id = properties.get("dictionary", 0)
        detector = self._get_detector(dict_id)
        if detector is None:
            return {"image": image, "result_json": "[]", "__error__": "Failed to create ArUco detector"}
