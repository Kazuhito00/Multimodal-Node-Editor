...
class VolumeSoftNodeLogic(ComputeLogic):
    """
    音量をスケールで調整するノードロジック（ソフトリミット版）。
    tanhを使用してソフトクリッピングを適用。
    """
    ...
    def compute(
        ...
        # deltaにスケールを適用し、tanhでソフトクリップ（-1.0〜1.0）
        delta = audio_in.get("delta", [])
        scaled_array = np.tanh(np.array(delta) * scale)
