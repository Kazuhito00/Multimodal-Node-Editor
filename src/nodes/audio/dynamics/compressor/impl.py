"""
Dynamic Range Compression ノードの実装。
閾値を超えた音声信号を指定比率で圧縮する。
"""
...
class DynamicRangeCompressionLogic(ComputeLogic):
    """
    ダイナミックレンジコンプレッションノードのロジック。
    閾値を超えた音声信号を指定比率で圧縮する。
    """

    def __init__(self):
        self._buffer: WaveformBuffer | None = None
        self._compressor: PeakCompressor | None = None
    ...
    def compute(
        ...
        # パラメータ取得
        threshold = float(properties.get("threshold", 0.5))
        ratio = float(properties.get("ratio", 4.0))
        ...
        if self._compressor is None or params_changed:
            self._compressor = PeakCompressor(
                threshold=threshold,
                ratio=ratio,
                attack_ms=attack_ms,
        ...
        if len(delta) > 0:
            ...
            compressed = self._compressor.process(delta_array)
            compressed_delta = compressed.tolist()
        else:
            compressed_delta = []

        # 圧縮済みdeltaをバッファに蓄積
        self._buffer.write(compressed_delta)
