"""
Highpass Filter ノードの実装。
指定したカットオフ周波数より低い周波数成分を除去する。
"""
...
class HighpassFilter:
    ...
    def __init__(
        ...
        normalized_cutoff = max(cutoff_freq / nyquist, 0.01)
        normalized_cutoff = min(normalized_cutoff, 0.99)

        # Butterworthフィルター係数を計算（SOS形式）
        self.sos = butter(filter_order, normalized_cutoff, btype='high', output='sos')
...
class HighpassFilterLogic(ComputeLogic):
    """
    ハイパスフィルターノードのロジック。
    指定したカットオフ周波数より低い周波数成分を除去する。
    """
    ...
    def __init__(self):
        ...
        self._filter: HighpassFilter | None = None
    ...
    def compute(
        ...
        # パラメータ取得（ポート入力を優先）
        cutoff_freq = inputs.get("cutoff_freq")
        if cutoff_freq is None:
            cutoff_freq = int(properties.get("cutoff_freq", 4000))
        else:
